...
// Timeout for PostgreSQL connections, configurable via environment variables
 ...
 * Ensures all necessary fields for PostgreSQL and SSH configurations are provided.
...
/**
 * Creates a PostgreSQL connection with a timeout for connection attempts.
 * @param {Object} config - Configuration for PostgreSQL.
 * @param {number} [localPort=null] - Local port for SSH tunneling.
 * @returns {Promise<Client>} The PostgreSQL client instance.
 */
 ...
 * @param {string} query - PostgreSQL query to execute.
 * @returns {Promise<Client>} The PostgreSQL client instance.
 */
const getOrCreateConnection = async (config, query) => {
  ...
  try {
    ...
    const connection = use_ssh
      ...
      : await createPostgresConnection(config);
 ...
 * Processes PostgreSQL queries based on provided configurations and query string.
